...
namespace BlazorSozluk.WebApp.Infrastructure.Services
{
    public class EntryService : IEntryService
    {
        ...
        {
            var result = await client.GetFromJsonAsync<List<GetEntriesViewModel>>("/api/entry?todaysEntites=false&count=30");
            return result;
        }
        ...
        {
            var res = await client.PostAsJsonAsync("/api/Entry/CreateEntry", createEntryCommand);
        ...
        public async Task<Guid> CreateEntryComment(CreateEntryCommentCommand createEntryCommentCommand)
        {
            ...
            var res = await client.PostAsJsonAsync("/api/Entry/CreateEntryComment", createEntryCommentCommand);
            if (!res.IsSuccessStatusCode)
            {
                return Guid.Empty;
            }
        ...
        public async Task<List<SearchEntryViewModel>> SearchBySubject(string searchSubject)
        {
            var result = await client.GetFromJsonAsync<List<SearchEntryViewModel>>(
              $"/api/entry/Search?searchText={searchSubject}"
                );
            return result;
        }
    }
